...
# Sidebar for Avatar / Image Upload (Using Pillow)
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        st.image(image, caption="Aapka AI Avatar", use_column_width=True)
...
for i, message in enumerate(st.session_state.messages):
    with st.chat_message(message["role"]):
        ...
        # Agar assistant ka message hai toh sath speak button bhi ho
        if message["role"] == "assistant":
            ...
            " style="background-color: #ff0033; color: white; border: none; padding: 6px 12px; border-radius: 5px; cursor: pointer; font-weight: bold; margin-top: 5px;">
                🔊 Listen Voice
            </button>
            """
            st.components.v1.html(speech_html, height=45)
